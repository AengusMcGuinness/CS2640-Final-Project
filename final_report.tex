...
Before plotting, I ran a consistency check over the final trial directories.
Each of the three trials contained TCP, two-sided RDMA, one-sided RDMA,
one-sided RDMA with metadata, CPU, and network-counter CSVs.  Every benchmark
CSV covered the same client counts, 1, 2, 4, and 8, and every aggregated
summary row reported \texttt{trial\_count=3}.  This check is important because
the final plots combine multiple scripts and data transfers from CloudLab.
