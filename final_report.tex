...
95\% GET ratio, and Zipf skew $s=0.8$.  The one-sided read benchmark measured
the proposal's direct-read path: GET-only lookups over the preloaded RDMA slot
array.  This means the three-way comparison should be read as a cache-read
comparison, not as a claim that one-sided RDMA currently replaces a full mixed
GET/SET workload.
...
benchmark hot paths did not include instrumentation.  Even at 100,000
operations per point, the fastest one-sided runs are short enough that CPU
sampling is still more useful qualitatively than as a precise utilization
measurement.
...
report treats Linux netdev byte counters as inconclusive and does not use the
network-byte plot as evidence.
...
from 12~$\mu$s to 15--16~$\mu$s.  The added p99 cost is therefore
3--4~$\mu$s.

\begin{figure*}[t]
\centering
\includegraphics[width=0.49\textwidth]{plots/final_summary_mlx5_3_100k/metadata_overhead/mean_latency_vs_clients_errorbars.png}
\hfill
\includegraphics[width=0.49\textwidth]{plots/final_summary_mlx5_3_100k/metadata_overhead/p99_latency_vs_clients_errorbars.png}
...
operation adds a consistent latency penalty, which is the central cost
predicted by the proposal.}
...
the client-count sweep.  This is less fundamental than the latency result,
because throughput is sensitive to client-side scheduling, but it reinforces
the same conclusion: metadata atomics are visible in the one-sided read path.
...
samples because the server is not on the read data path after setup.  The
measurement has an important caveat: many RDMA benchmark runs are short enough
...
files, and generated plots.  The primary final inputs are the raw trials under
\texttt{experiments/final\_trials\_mlx5\_3\_100k/}, the aggregated CSVs under
\texttt{experiments/final\_summary\_mlx5\_3\_100k/}, and the figures under
\texttt{plots/final\_summary\_mlx5\_3\_100k/}.
